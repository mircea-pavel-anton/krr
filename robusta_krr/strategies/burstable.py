...
class BurstableStrategySettings(StrategySettings):
    ...
    cpu_limit_percentile: float = pd.Field(
        99, gt=0, le=100, description="The percentile to use for the CPU limit."
    )
    memory_request_percentile: float = pd.Field(
        50, gt=0, le=100, description="The percentile to use for the memory request."
    )
    memory_limit_percentile: float = pd.Field(
        99, gt=0, le=100, description="The percentile to use for the memory limit."
    )
    ...
    def calculate_percentile(self, data: PodsTimeData, percentile: float) -> float:
        if len(data) == 0:
            return float("NaN")
        if len(data) > 1:
        ...
        return float(np.percentile(data_, percentile))
...
class BurstableStrategy(BaseStrategy[BurstableStrategySettings]):
    ...
    def description(self):
        s = textwrap.dedent(f"""\
            CPU request: {self.settings.cpu_request_percentile}% percentile, limit: {self.settings.cpu_limit_percentile}% percentile
            Memory request: {self.settings.memory_request_percentile}% percentile, limit: {self.settings.memory_limit_percentile}% percentile + {self.settings.memory_buffer_percentage}% buffer
    ...
    def __calculate_cpu_proposal(
        ...
        cpu_request = self.settings.calculate_percentile(data, self.settings.cpu_request_percentile)
        cpu_limit = self.settings.calculate_percentile(data, self.settings.cpu_limit_percentile)
        return ResourceRecommendation(request=cpu_request, limit=cpu_limit)

    def __calculate_memory_proposal(
        self, history_data: MetricsPodData, object_data: K8sObjectData
    ) -> ResourceRecommendation:
        data = history_data["MemoryLoader"]
        ...
        memory_request = self.settings.calculate_percentile(data, self.settings.memory_request_percentile)
        memory_limit = max(
            self.settings.calculate_percentile(data, self.settings.memory_limit_percentile)
            * (1 + self.settings.memory_buffer_percentage / 100),
            max_oomkill_value * (1 + self.settings.oom_memory_buffer_percentage / 100),
        )
        # Ensure limit is never below request
